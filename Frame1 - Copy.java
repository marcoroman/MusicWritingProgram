...
public class Frame1 {
	...
	public static void main(String[] args) throws IOException
	{
		...
		try{
			for(int i = 1; i<22; i++)
			{
				...
				clip.open(AudioSystem.getAudioInputStream(new File("C:\\Users\\duyng\\workspace\\Synthia\\sounds\\loud\\" + i + ".wav")));
				softClips.add(clip);
	...
	public void start() throws IOException, LineUnavailableException, UnsupportedAudioFileException, InterruptedException{
		...
		drums.open(AudioSystem.getAudioInputStream(new File("C:\\Users\\duyng\\workspace\\Synthia\\sounds\\d.wav")));
	...
	public void initializeTimerTask() {
		timerTask = new TimerTask() {
			public void run() {
				...
				int volume = tm.getVolume(counter/16);
				...
				if (pitch == 0){
					if (softClips.get(currentPosition).isRunning())
					{
						...
						softClips.get(currentPosition).setMicrosecondPosition(100);
					}
				}
				else if (pitch != 25){
					softClips.get(pitch-1).setMicrosecondPosition(100);
					FloatControl gainControl = 
						    (FloatControl) softClips.get(currentPosition).getControl(FloatControl.Type.MASTER_GAIN);
					switch (volume){
						case -2: 
